...
def test_call_map(observer, obs_time, model):
    ...
    executor.set_measurement(CloudCoverageInfo(np.identity(3), 0, 0, 0, 0, 1, 2, 0.1, obs_time))
...
def test_call_map_nan(observer, obs_time, model):
    ...
    executor.set_measurement(CloudCoverageInfo(cloud_map, 0, 0, 0, 0, 1, 2, 0.1, obs_time))
